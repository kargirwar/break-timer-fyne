...
import (
	...
	utils "github.com/kargirwar/break-timer/ctl/utils"
)

const RELEASE_ARCHIVE = "release.zip"
const BINARY = "break-timer"
const LABEL = "com.kargirwar.break-timer"
...
func StartAgent() {
	...
	data := struct {
		...
		Label:   LABEL,
		Program: "/usr/local/bin/break-timer",
	}
...
func CopyAgent() {
	...
	program := utils.GetCwd() + "/../break-timer"
...
func CopyMedia() {
	fmt.Println("Copying audio file to /usr/local/bin ...")
	program := utils.GetCwd() + "/../beep.mp3"
...
func DelAgent() {
	...
	//copy executable to /usr/local/bin
	cmd := exec.Command("rm", "-f", "/usr/local/bin/break-timer")
	err := cmd.Run()
